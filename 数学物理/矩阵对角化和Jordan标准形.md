...
则上面的式子也可以化为和式$$A=\lambda_{1}v_{1}v_{1}^T+\lambda_{2}v_{2}v_{2}^T+\dots+\lambda_{n}v_{n}v_{n}^T$$这被称为矩阵的*谱分解*。特征值是离散的，而谱可以是连续的。
...
- 一个线性映射的最小多项式是唯一的。
- 若$m(\lambda)$是$\mathbf{A}$的一个最小多项式，则$m(\lambda)|g(\lambda)\Leftrightarrow g(\lambda)$是零化多项式。
- 相似的矩阵有相同的最小多项式。
...
判断一个未知矩阵$A$的Jordan标准形的步骤：
1. 计算特征多项式$f(\lambda)=\det (A-\lambda I)$，分解为一次因式的乘积$$f(\lambda)=(\lambda-\lambda_1)^{r_1}(\lambda-\lambda_2)^{r_2}\cdots(\lambda-\lambda_s)^{r_s}$$记录每个特征值$\lambda_{i}$和代数重数$r_{i}$。
2. 对每一个特征值$\lambda_{i}$，设幂零化矩阵为$N_{i}=A-\lambda_{i}I$，依据以下递推公式计算序列$\{ d_{k} \}$$$d_{k}=\dim \mathrm{Ker}N^k=n-\mathrm{rank}(N^k),\quad k=1,2,\dots$$其中$d_{1}$是几何重数。序列单调非减，会逐步从几何重数增大并稳定到代数重数。
3. 属于$\lambda_{i}$的、阶数$\geq k$的Jordan块个数为$d_{k}-d_{k-1}$，由此便能判定各阶Jordan块的数量。
4. 对每一个特征值$\lambda_{i}$重复2,3操作，就能得到Jordan标准形的完整结构。