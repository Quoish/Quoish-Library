...
对于方形镜$$u_{mn}(x,y,z)=\frac{1}{2^{m+n}m!n!} \frac{2}{\sqrt{ \lambda L }} \frac{w_{0}}{w(z)}H_{m}\left( \frac{\sqrt{ 2 }}{w(z)}x \right)H_{n}\left( \frac{\sqrt{ 2 }}{w(z)}y \right)\exp\left(- \frac{r^2}{w^2(z)} -i(m+n+1)\psi(z)\right)$$对于圆形镜$$u_{mn}(\rho,\varphi,z)=\frac{1}{w(z)}\left( \frac{\sqrt{ 2 }r}{w(z)} \right)^n$$
其中：
- 束腰半径：$$$$
