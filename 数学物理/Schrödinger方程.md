#量子力学 
# Schrödinger方程
我们注意到单色平面经典波是$Ae^{ i(\vec{k}\cdot r-\omega t) }$的实部，于是不妨将能量$E=\hbar \omega$，动量$\vec{p}'=\hbar \vec{k}'$，质量为$m$的非相对论自由粒子波函数写为
...
4. 设$V(-x)=V(x)$,则对应于任何一个能量本征值$E$，总可以找到方程的一组完备的解，它们中每一个都具有确定的宇称(奇偶性)。(注意:每一个解的宇称并不一定相同)
5. 对于一维运动粒子,设$ψ_{1}(x)$与$ψ_{2}(x)$是方程的属于能量本征值$E$的两个解,则$\psi_1\psi_2'-\psi_2\psi_1'=\text{Const}$
6. 设$V(x)$是规则势场,如存在束缚态,则必定是不简并的。