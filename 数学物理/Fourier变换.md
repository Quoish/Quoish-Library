...
当两个频率分别为$\omega_{0}=\dfrac{2\pi}{14}$和$\omega_{1}=\dfrac{4\pi}{15}$时，其64点DFT幅度| V[k]|如图6.5-6(c)所示，
它只取到了|V（e"）|的峰值和零值。这两种情况
都使采用V[k]估计V(e"）存在很大偏差。增加
DFT的点数N能够取样到V(e"）更多的细节，因
此常常使用时域补零的方法对傅里叶变换充分地
过采样，以便将一些重要的频域细节表现出来。
当DFT点数足够大时，通过对V[k]的线性内插就
能得到对V(e"）相当精确的估计。所以，一般来
说，DFT点数N大于等于前面讨论的时域窗长
(M+1)。