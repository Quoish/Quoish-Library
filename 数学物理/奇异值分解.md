...
# 奇异值分解的计算
用上面的分解式计算$A^TA$可以得到$$A^TA=V\Sigma U^TU\Sigma V^T=V\Sigma^2V^T=V\Sigma^2V^{-1}$$所以可以用[[矩阵对角化和Jordan标准形]]的方式计算奇异值分解。
...
# 奇异值谱分解
秩为$r$的矩阵$A$分解为$A=U\Sigma V^T$，若设$U=(u_{1},u_{2},\dots,u_{n}),V=(v_{1},v_{2},\dots,v_{n})$，各个奇异值记为$\sigma_{i}$，则可以写成和式$$A=\sum_{i}^r \sigma_{i}u_{i}v_{i}^T$$
## 低秩近似
既然$A = \sum_{i=1}^{r} \sigma_i u_i v_i^T$，如果我们只取前$k$个最大的奇异值及其对应的向量，就可以得到一个近似的矩阵$A_{k}$：$$A \approx A_k = \sum_{i=1}^{k} \sigma_i u_i v_i^T$$Eckart-Young定理指出，在所有秩为$k$的矩阵中，$A_{k}$是原矩阵$A$在 Frobenius 范数意义下的最佳近似。