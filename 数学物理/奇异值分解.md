...
## 低秩近似
既然$A = \sum_{i=1}^{r} \sigma_i u_i v_i^T$，如果我们只取前$k$个最大的奇异值及其对应的向量，就可以得到一个近似的矩阵$A_{k}$：$$A \approx A_k = \sum_{i=1}^{k} \sigma_i u_i v_i^T$$Eckart-Young定理指出，在所有秩为 \( k \) 的矩阵中，\( A_k \) 是原矩阵 \( A \) 在 Frobenius 范数意义下的最佳近似。