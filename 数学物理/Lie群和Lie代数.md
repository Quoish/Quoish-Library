...
指数映射建立起了生成元与单参数子群的联系，例如平移算符$T_{a}f(x)=f(x+a)$，那么$T_{a}=e^{a\frac{ \mathrm{d}   }{ \mathrm{d}  x} }$；
# 常见Lie群和Lie代数
## 一般线性群
域$F$上$n$阶可逆矩阵的集合$GL_{n}(F)$对于矩阵乘法构成一般线性群。它可以视为$\mathbb{R}^{n^2}$的Lie子群，它包括行列式大于0和行列式小于0的两部分元素，是个非连通群。
