...
>设$\mathcal{H}\subseteq\mathcal{G}$是理想的，则有商集$\mathcal{G}/\mathcal{H}$是Lie代数，称为商Lie代数。
# 单参数子群和指数映射
>光滑曲线$\gamma:\mathbb{R}\to G$若满足$$\gamma(s+t)=\gamma(s)\cdot\gamma(t),\quad \forall s,t \in \mathbb{R}$$则称$\gamma$是Lie群$G$的*单参数子群*，并且$\gamma$是$\mathbb{R}$到$G$的Lie群同态。
