...
设$\phi:\mathbb{R}\times M\to M$是单参数微分同胚群，那$\phi_{p}:\mathbb{R}\to M,p \in M$就是一条曲线，叫做该单参数微分同胚群过$p$点的*轨道*（Orbit）。不同的$p$给出不同的曲线，这些曲线的切向量可以给出一个矢量场，所以*一个单参数微分同胚群给出一个矢量场*。但是反过来并不一定乘积