...
## 单参数微分同胚群
在$\mathbb{R}^3$中，设整个系统向$x$轴平移$a$的映射为$\phi_{a}:\mathbb{R}^3\to \mathbb{R}^3$，所有不同的实数$a$诱导的平移映射$\phi_{a},a\in \mathbb{R}$可以构成一个群$G$，这个群的所有元素都可以用一个单数$a$表征，所以称$G$为单参数群（One-Parameter Group）。