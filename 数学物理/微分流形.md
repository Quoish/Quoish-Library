...
设$\phi:\mathbb{R}\times M\to M$是单参数微分同胚群，那$\phi_{p}:\mathbb{R}\to M,p \in M$就是一条曲线，叫做该单参数微分同胚群过$p$点的*轨道*（Orbit）。不同的$p$给出不同的曲线，这些曲线的切向量可以给出一个矢量场，所以*一个单参数微分同胚群给出一个矢量场*。但是反过来并不一定成立，矢量场虽然能靠积分曲线给出流形上的曲线，但是正如上面一节说的，这些曲线可能是不可延的，因此就没法构成一个群，只能构成一个局部群，因此一个矢量场给出一个单参数微分同胚局部群。