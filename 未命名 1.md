拓扑空间$(M,\mathscr{T})$若有开覆盖$\{ O_{\alpha} \}_{\alpha \in I}$，即$M=\bigcup_{\alpha \in I}O_{\alpha}$，且$\forall O_{\alpha},\exists \varphi_{\alpha}:O_{\alpha}\to V_{\alpha}$是同胚映射，而$V_{\alpha}$是$\mathbb{R}^n$在某一拓扑$\mathscr{S}$下的开集，则称$M$是$n$维流形。$$f:X_{1}\to Y_{1},g:X_{2}\to Y_{2},h\circ f=g,h:Y_{1}\to X_{2}$$$$\frac{ \partial  D_{i} }{ \partial  E_{j}} $$
